...
void ParticleFilter::dataAssociation(Map map_in_range_landmarks, std::vector<LandmarkObs>& observations) {
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[],
	...
	double x_landmark, y_landmark; // landmarks coordinate
	...
	for(unsigned int pi=0; pi < num_particles_ ; pi++){
		...
		// Declare map object
		Map map_obs; // for storing observations in global coordinate
		Map map_in_range_landmarks; // for storing landmarks in sensor range
		...
		for(unsigned int mi=0; mi < map_landmarks.landmark_list.size(); mi++){
			x_landmark = map_landmarks.landmark_list[mi].x_f;
			y_landmark = map_landmarks.landmark_list[mi].y_f;
			if(dist(x_p, y_p, x_landmark, y_landmark) < sensor_range){
				map_in_range_landmarks.landmark_list.push_back(map_landmarks.landmark_list[mi]);
			}
		}

		// store assosiating result
		...
		for(unsigned int zi=0; zi < observations.size(); zi++){
			...
			// find associations
			dist_min = sensor_range; // initialize
			particles_[pi].weight = 1.0;
			for(unsigned int mi=0; mi < map_in_range_landmarks.landmark_list.size(); mi++){
				x_landmark = map_in_range_landmarks.landmark_list[mi].x_f;
				y_landmark = map_in_range_landmarks.landmark_list[mi].y_f;
				tmp_dist = dist(tmp_landmark_obs.x_f, tmp_landmark_obs.y_f, x_landmark, y_landmark);
				// update minimum range landmark
				if(dist_min > tmp_dist){
					dist_min = tmp_dist;
					map_obs.landmark_list[zi].id_i = map_in_range_landmarks.landmark_list[mi].id_i;
					tmp_landmark_NN = map_in_range_landmarks.landmark_list[mi];
					}
				}
				...
				// update associations
				sense_x.push_back(map_obs.landmark_list[zi].x_f);
				sense_y.push_back(map_obs.landmark_list[zi].y_f);
				associations.push_back(tmp_landmark_NN.id_i);
				/*
			...
			// store associations
			SetAssociations(particles_[pi], associations, sense_x, sense_y);
		}
