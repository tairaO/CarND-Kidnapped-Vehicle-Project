...
/*
 * Computes Multivariate gaussian probability of 2D
 * @param (x_obs,y_obs)     x and y coordinates of obseravation
 * @param (mu_x,mu_y)       mean of x and y
 * @param (sigma_x,sigma_y) standard deviations od x and y
 * @output Multivariate gaussian probability of 2D
 */
inline double multivariate_gauss_prob(double x_obs, double y_obs, double mu_x, double mu_y, double sigma_x, double sigma_y) {
	// calculate normalization term
	double gauss_norm = (1.0/(2.0 * M_PI * sigma_x * sigma_y));
	// calculate exponent
	double exponent = (pow((x_obs-mu_x),2.0) / (2.0 * pow(sigma_x,2.0))) + (pow((y_obs - mu_y),2.0)/(2.0 * pow(sigma_y,2.0)));
